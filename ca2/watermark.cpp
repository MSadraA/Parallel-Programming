...
int main() {
    ...
    for(int i=0; i<3; ++i) {
        serial_optimized_blend_channel(base_channels[i], water_channels[i], out_serial_channels[i]);
    }
    end = ippGetCpuClocks();
    time2 = end - start;
    ...
    for(int i=0; i<3; ++i) {
        parallel_blend_avx_channel(base_channels[i], water_channels[i], out_parallel_channels[i]);
    }
