...
void* process_worker(void* args) {
    ...
    std::cout << "[Process] OpenMP Worker started." << std::endl;
    // Print number of threads available
    std::cout << "[Process] Max OpenMP threads: " << omp_get_max_threads() << std::endl;
    ...
    while (true) {
        ...
        // 2. Convert to Grayscale (Fast enough, usually not worth parallelizing)
        cv::Mat gray;
        cv::cvtColor(frame, gray, cv::COLOR_BGR2GRAY);

        // Prepare matrices
        cv::Mat grad_x, grad_y;
        cv::Mat detected_edges = cv::Mat::zeros(gray.size(), CV_8U);
        ...
        // --- OPENMP SECTION 2: Data Parallelism ---
        // Instead of using cv::convertScaleAbs and cv::addWeighted, 
        // we implement the pixel merging manually to demonstrate "parallel for".
...
int main() {
    ...
    output_args.filename = "output_sobel_parallel.avi";
